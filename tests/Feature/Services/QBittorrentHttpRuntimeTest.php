...
class QBittorrentHttpRuntimeTest extends TestCase
{
    ...
    public function test_real_http_transport_reuses_cached_sid_and_adds_magnet(): void
    {
        ...
        $this->assertSameOriginHeaders($requests, $port);
    }
    ...
    /**
     * qBittorrent 4.1-4.6 documents that Web API calls must provide an
     * Origin or Referer matching the request Host domain and port.
     *
     * @param  array<int, array<string, mixed>>  $requests
     */
    private function assertSameOriginHeaders(array $requests, int $port): void
    {
        $expectedOrigin = 'http://127.0.0.1:'.$port;

        foreach ($requests as $request) {
            $origin = (string) ($request['origin'] ?? '');
            $referer = rtrim((string) ($request['referer'] ?? ''), '/');

            $this->assertTrue(
                $origin === $expectedOrigin || $referer === $expectedOrigin,
                sprintf(
                    'Expected matching Origin/Referer for %s; got Origin=%s Referer=%s',
                    (string) ($request['path'] ?? ''),
                    $origin,
