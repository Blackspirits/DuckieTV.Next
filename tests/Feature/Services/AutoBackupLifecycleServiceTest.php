...
class AutoBackupLifecycleServiceTest extends TestCase
{
    ...
    public function test_daily_schedule_preserves_local_wall_clock_across_dst(): void
    {
        settings('autobackup.period', 'daily');

        $timezone = new DateTimeZone('Europe/Lisbon');
        $lastRun = (new DateTimeImmutable('2026-03-28 12:00:00', $timezone))->getTimestamp() * 1000;
        $expectedNext = (new DateTimeImmutable('2026-03-29 12:00:00', $timezone))->getTimestamp() * 1000;

        settings('autobackup.lastrun', $lastRun);

        $status = app(AutoBackupLifecycleService::class)->status(
            $lastRun,
            'Europe/Lisbon'
        );

        $this->assertSame($expectedNext, $status['next_run_ms']);
        $this->assertSame(23 * 60 * 60 * 1000, $expectedNext - $lastRun);
    }
