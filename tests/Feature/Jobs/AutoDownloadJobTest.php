...
it('keeps the dedicated auto download queue on the managed NativePHP worker', function () {
    expect(AutoDownloadLifecycleService::QUEUE)->toBe('autodownload')
        ->and(config('nativephp.queue_workers.default.queues'))
        ->toBe(['autodownload', 'default']);
});
