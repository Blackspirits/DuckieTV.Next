    <div class="buttons" data-section="torrent-search">
        ...
        <a href="javascript:void(0)" onclick="setSearchQuality('')" style="padding:10px; height:45px; margin: 2px;" class="btn {{ $currentQuality == '' ? 'btn-success' : '' }}">
            @if($currentQuality == '')
                <i class="glyphicon glyphicon-ok"></i>
            @endif
            <strong style='position:absolute; left:60px'>{{ __('SETTINGS/TORRENT-SEARCH/quality-select-all/lbl') }}</strong>
        </a>
        ...
        <ul class="list-unstyled">
            <li>
                ...
                <a href="javascript:void(0)" onclick="toggleTorrentSearchSetting('torrenting.require_keywords_mode_or', {{ $requireKeywordsModeOR ? 'false' : 'true' }})" class="btn btn-{{ $requireKeywordsModeOR ? 'info' : 'success' }}">
        ...
        <ul class="list-unstyled">
            <li>
                ...
                <input type='text' style='width:350px' id="requireKeywords" value="{{ settings()->get('torrenting.require_keywords', '') }}" placeholder="{{ __('COMMON/global-search/placeholder') }}">
                <a class="btn btn-success" onclick="saveTorrentSearchSetting('torrenting.require_keywords', document.getElementById('requireKeywords').value)">
        ...
        <ul class="list-unstyled">
            <li>
                ...
                <input type='text' style='width:350px' id="ignoreKeywords" value="{{ settings()->get('torrenting.ignore_keywords', '') }}" placeholder="{{ __('COMMON/global-search/placeholder') }}">
                <a class="btn btn-success" onclick="saveTorrentSearchSetting('torrenting.ignore_keywords', document.getElementById('ignoreKeywords').value)">
                    <i class="glyphicon glyphicon-floppy-save"></i>&nbsp; {{ __('COMMON/save/btn') }}
                </a>
            </li>
        </ul>
        ...
        <div class="autodownload">
            ...
            <input type="number" id="minSeeders" value="{{ settings()->get('torrenting.min_seeders', 50) }}" min="0" max="3000">
            <a class="btn btn-success" onclick="saveTorrentSearchSetting('torrenting.min_seeders', Number(document.getElementById('minSeeders').value))" style="float:right; margin-top:-10px;">
                <i class="glyphicon glyphicon-floppy-save"></i>&nbsp; {{ __('COMMON/save/btn') }}
            </a>
        </div>
        ...
        <form>
            ...
            <input type="number" min="0" id="globalSizeMin" value="{{ settings()->get('torrenting.global_size_min') }}" placeholder="{{ __('COMMON/search-size/placeholder') }}">
            <a class="btn btn-success" onclick="saveTorrentSearchSetting('torrenting.global_size_min', numericSettingOrNull('globalSizeMin'))" style="float:right; margin-top:-10px;">
                <i class="glyphicon glyphicon-floppy-save"></i>&nbsp; {{ __('COMMON/save/btn') }}
            </a>
        </form>
        ...
        <form>
            <span>{{ __('COMMON/global-size/form') }}</span>
            <input type="number" min="0" id="globalSizeMax" value="{{ settings()->get('torrenting.global_size_max') }}" placeholder="{{ __('COMMON/search-size/placeholder') }}">
            <a class="btn btn-success" onclick="saveTorrentSearchSetting('torrenting.global_size_max', numericSettingOrNull('globalSizeMax'))" style="float:right; margin-top:-10px;">
                <i class="glyphicon glyphicon-floppy-save"></i>&nbsp; {{ __('COMMON/save/btn') }}
            </a>
        </form>
    </div>
