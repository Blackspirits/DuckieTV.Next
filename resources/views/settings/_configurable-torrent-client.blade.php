@php
    $hasPath = $hasPath ?? false;
    $hasToken = $hasToken ?? false;
    $hasAuthToggle = $hasAuthToggle ?? false;
    $hasCredentials = $hasCredentials ?? false;
    $authEnabled = $hasAuthToggle
        ? (bool) settings()->get($prefix.'.use_auth', false)
        : true;
@endphp
...
<div class="body">
    ...
    <form method="POST" action="{{ route('settings.update', 'torrent') }}" data-section="torrent" onsubmit="Settings.test('torrent'); return false;">
        ...
        @if($hasToken)
            <div class="form-group">
                <label>Token</label>
                <input type="password" name="{{ $prefix }}.token" class="form-control" value="" autocomplete="new-password" placeholder="Leave blank to keep the saved token">
            </div>
        @endif
